...
const Dashboard = () => {
  ...
  const createResume = async (e) => {
    ...
    try {
      ...
      navigation(`/app/builder/${data.resume._id}`)
    } catch (error) {
      toast.error(error?.response?.data?.message || error.message)
    }
  ...
  const uploadResume = async (e) => {
    ...
    try {
      ...
      setResume(null);
      setShowCreateResume(false);
      navigation(`/app/builder/${data.resumeId}`);
    ...
    return (
      <div>
        <div className='max-w-7xl mx-auto px-4 py-8'>
          <p className='text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to bg-slate-700 bg-clip-text text-transparent sm:hidden'>Welcome, {user.name}</p>
          ...
          <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
            {allResumes.map((resume, index) => {
              ...
              return (
                <button onClick={() => navigation(`/app/builder/${resume._id}`)} key={index} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300' style={{ background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor: baseColor + '40' }}>
          ...
          {showUploadResume && (
            <form onSubmit={uploadResume} onClick={() => setShowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center '>
              ...
              <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors flex items-center gap-1 justify-center disabled:cursor-not-allowed opacity-80' disabled={isLoading}>
